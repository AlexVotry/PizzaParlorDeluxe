<?php
// start a session
session_start();// Create a variable to hold the correct referring page URL This value will need to be changed when the site is moved to the testing server and to the production server
	$httpReferer = "http://localhost/pizzaParlorDeluxe/admin/changePass.php";
...
// if the user is not authenticated, send them back to the log in page with a warning
	if (!$_SESSION['loggedIn']){
	    $_SESSION["loginWarn"] = "You must be logged into the system to access the page. not logged in";
    ...
    // requiring the db connection file, this file must exist and included only once
 	     ...
 	     $sql = "SELECT password FROM admins WHERE adminID = " . $_SESSION['adminID'] . ";";
 	...
 	// check to see if data was returned from the query
	     ...
	     // else check stored, hashed password against entered, hashed password
     ...
     if ($oldPassCorrect){
        ...
        if ($_POST['pswWord'] == $_POST['pswConf']){
	     ...
	     if ($oldPassCorrect && $newPassCorrect){
	         ...
	         $sql = "UPDATE admins SET password = '". $hashPass . "' WHERE adminID = " . $_SESSION['adminID'] . ";";
